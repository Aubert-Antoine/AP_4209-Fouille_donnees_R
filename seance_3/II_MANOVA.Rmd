...
```{r}
# MANOVA_DATASET <- read_csv("./seance_3/MANOVA_DATASET.csv")
...
```{r}
# df_MANOVA <- read("./MANOVA_DATAS ET.Rda")
load("./seance_3/MANOVA_DATASET.Rda")
```
...
MANOVA_DATASET %>% kbl(digits=3) %>%
  kable_material_dark(bootstrap_options = "striped", full_width = T, position = "center", latex_options = 'stripped')
```
...
```{r P}
# à compléter
numeric_col <- which(sapply(MANOVA_DATASET, is.numeric))
P <- length(numeric_col)
```
...
Au final, la matrice (*SS_tot*) est la suivante :

```{r}
# Convert the dataframe to a matrix
X_mat <- as.matrix(X)
G_mat <- matrix(G, nrow = nrow(X_mat), ncol = ncol(X_mat), byrow = TRUE)
dif <- X_mat - G_mat
SS_TOT <- t(dif) %*% dif
...
```{r}
# à compléter
# Divide the data into groups based on the "Class" column


# Calculate the matrix of differences for each group
...
# Calculate the sum of squares of the differences for each group
...
https://rpubs.com/learstudio/988960