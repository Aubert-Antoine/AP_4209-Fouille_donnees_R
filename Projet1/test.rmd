...
```{r}
# pred <- predict(model, data.frame(Comment = "No ! It's false, he'd make shit"))
# pred <- predict(model, data.frame(Comment = "Yes it is lovely"))
pred <- predict(model, data.frame(Comment = "I am not sure about that, I think it is dangerous"))
pred
```