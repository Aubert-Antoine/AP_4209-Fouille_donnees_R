...
corpus <- create_corpus(dataset, clean_text, stem_strings)
...
```
```{r}
model <- naiveBayes(dataset_vec$train_commentaire, dataset_vec$train_emotion)
# model <- naiveBayes(as.matrix(), train_emotion)
...
```{r}
predic_commentaire <- predic_test(model = model, comment = Comment, corpus_tfidf_df = corpus_tfidf_df, clean_text_func = clean_text, vec_string_methode = stem_strings)
```
