...
corpus_lemmatize <- tm_map(corpus, lemmatize_strings)

# Inspect the lemmatized corpus
head(corpus_lemmatize$content$Comment)
```
...
```{r}
dtm <- DocumentTermMatrix(corpus_lemmatize$content$Comment)
...
train_indices <- sample(1:nrow(corpus_tfidf_df), size = 0.7 * nrow(corpus_tfidf_df))
...
train_emotion <- corpus_lemmatize$content$Emotion[train_indices]
test_emotion <- corpus_lemmatize$content$Emotion[-train_indices]
```
...
cm <- confusionMatrix(predictions, test_emotion)
